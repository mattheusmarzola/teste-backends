...
import com.bcredi.testebackend.gateway.EventGateway;
import com.bcredi.testebackend.gateway.ProposalGateway;
...
public class Solution {
	...
	public static String processMessages(List<String> messages) {
		...
		ProposalGateway proposalGateway = new ProposalGateway();
		
		for (int i = 0; i <= messages.size() - 1; ++i) {
			String message = messages.get(i);
			try {
				Event event = eventGateway.createEvent(message);
				if(event.getEventAction().isValid()) {
					event.getEventAction().process();
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		List<Proposal> allValidProposal= proposalGateway.getAllValidProposal();
		List<String> proposalIds = allValidProposal.stream().map(p -> p.getId()).collect(Collectors.toList());
		
		EventListSingleton.cleanData();
		ProposalListSingleton.cleanData();
		
		return String.join(",", proposalIds);
	}
